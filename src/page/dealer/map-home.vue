  ...
  export default{
    data () {
      return {
        ...
        agcy400TelStyle:'width: 3.2rem;font-size: .22rem;line-height: .34rem;color: #666;margin-top: .07rem;',
        dealerDate:'',
        dealers: [],
        cityId: '',
        longitude: '',
    ...
    created(){
      ...
      this.longitude = this.$route.query.latitude;
      this.latitude = this.$route.query.longitude;
    ...
    methods: {
      tel400Z(){
         window.tel400Z =function(){
            var text = document.getElementById('agcy400Tel').innerHTML;
            console.log('呼叫 '+ text);
            var bridge = getJsBridge();
            bridge.call("callTel",{tel:text});
         }
      },
      returnback(){
        var bridge = getJsBridge();
      ...
      initMap (data) {
        ...
        this.$axios(this.dealerDate).then((response) => {
          this.dealers = response.response_data.dealers;
           this.agcy400Tel = this.dealers[0].ci400phone;
          for(var i=0;i<this.dealers.length;i++){
            var marker = new BMap.Marker(new BMap.Point(this.dealers[i].latitude,this.dealers[i].longitude));  // 创建标注
            ...
            var content = '<div style="'+this.mapBox+'">'+
                            ...
                            '</a>'+
                            '<p style="' + this.agcy400TelStyle + '" onclick="tel400Z()" ><span>服务热线 : </span><span id="agcy400Tel">'+ this.agcy400Tel + '</span></p>' +
