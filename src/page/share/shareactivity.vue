...
<div>
	<div class="carDetail" v-show="!failed">
		...
		<div class="carSourceMessage">
			<div>
				<h2 class="activity_title">{{dealers.activityTitle}}</h2>
				<p class="activity_Content">{{dealers.activityDescribe}}
				</p>
				<p class="time">活动有效期：{{dealers.effectiveStartTime.replace(/-/g,'.')}}-{{dealers.endTime.replace(/-/g,'.')}}</p>
		...
		<div :class="{'cardetailFooter': true, 'sharecarfooter': share}">
			<ul>
				<li class="activity_Store" @click="goDelaer">进入店铺</li>
				<li class="activity_Online" @click="onlineConsulting">在线咨询</li>
			</ul>
		</div>
			...
			activityDetailsData(){
				...
				this.$axios(this.dealersData).then((response) => {
					if(response.retCode == "0"){
						...
						_this.shareTitle = "【活动】"+_this.dealers.activityTitle;	
