  ...
  <div v-show="!failed">
  	<div class="carDetail" v-show="!load">
		...
		<div class="navigation">
			<div class="left">
				<h3>{{agcyName}}</h3>
				<p @click="jumpmapNav">{{agcyAddress}}</p>
			</div>
			<a :href="$global().downloadApp">
				<div class="right"></div>
			</a>
		</div>
		...
		<div class="recommendForYou">
			...
			<div>
				<Carlistsm queryType="RECOMMEND" :cityId="cityId" :modelLine="modelLine" pageSize=2 @listenToChildEvent="recommend" content="正在研究您的喜好！" carDetailPath="sharecar"></Carlistsm>
			</div>
		</div>
			...
			carDetailHttpData(){
				...
				this.$axios(data).then((value) => {			
					if(value.retCode == 0){
						...
						_this.shareTitle = ("奥迪"+" "+_this.carModel+" "+_this.modelCode+" "+_this.transmission_Type+" "+_this.carAir+" "+_this.deviceType).replace(/\sundefined/g,'');	
						document.title = _this.shareTitle;
						let shareInfo = _this.carDetail.shareOfficial;	
						let shareUrl = this.$global().url+"sharecar?releaseNumber="+_this.releaseNumber+"&modelLine="+encodeURI(_this.modelLine)+"&cityId="+_this.cityId;
						// xxxx://xxxxx代表打开app的协议  
						_this.$share('xxxx://xxxxx?title=-1&uri='+_this.$global().url+'cardetail?releaseNumber='+_this.releaseNumber+"&modelLine="+encodeURI(_this.modelLine)+"&cityId="+_this.cityId+"&shareTitle="+_this.shareTitle+"&shareInfo="+shareInfo+"&shareImg="+_this.$global().shareImg+"&shareUrl="+shareUrl)
						setTimeout(() => {
							_this.MySwiper();
						},1)
